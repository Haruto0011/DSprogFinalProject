...
# 千代田区のSUUMOページURL
url = "https://suumo.jp/chintai/tokyo/sc_chiyoda/"
...
# スクレイピング処理（全ページ対応）
def scrape_chiyoda_all_pages(base_url):
    properties = []
    page = 1  # ページ数カウント用
    while base_url:
...
# スクレイピング開始
properties = scrape_chiyoda_all_pages(url)

# データをCSVに保存
if properties:
    df = pd.DataFrame(properties)
    df.to_csv("chiyoda_all_properties.csv", index=False, encoding="utf-8-sig")
    print("データをCSVに保存しました: chiyoda_all_properties.csv")
else:
